...
/**
 * Place this above any <ErrorBoundary> that should report route and wallet context.
 */
...
function ErrorFallbackUI({
  ...
  return (
    <div
      role="alert"
      className="bg-background flex min-h-screen items-center justify-center px-6 font-mono transition-colors duration-300"
    >
      ...
      <div className="relative z-10 w-full max-w-[600px]">
        {/* Main card */}
        <div className="bg-card border-border border px-8 pt-9 pb-7 shadow-xl">
          {/* Error header stripe */}
          <div className="bg-destructive border-border -mx-8 -mt-9 mb-7 flex items-center gap-2.5 border-b px-5 py-3">
            <span className="text-destructive-foreground text-xl font-semibold tracking-[4px]">
              ERROR
            </span>
            <span className="bg-card text-destructive ml-auto px-2 py-0.5 text-[11px] font-semibold tracking-[2px]">
              {kind.toUpperCase()}
            </span>
          </div>
          ...
          <div className="mb-4">
            ...
            <h1 className="text-[26px] leading-tight font-semibold tracking-tight">{copy.title}</h1>
          </div>
          ...
          {/* Error code block */}
          <div className="border-border mb-6 overflow-x-auto border bg-black px-3.5 py-3">
          ...
          <div className="mb-5 flex flex-wrap gap-3">
            <Button
              variant="secondary"
              onClick={onReset}
              className="flex-1 basis-[140px] font-semibold tracking-wider uppercase"
            >
              <RotateCcw size={14} />
          ...
          <a
            ...
            className="text-foreground border-foreground inline-flex items-center gap-1.5 border-b-2 pb-px text-xs font-semibold tracking-[0.5px]"
          >
        ...
        {import.meta.env.DEV && (
          <details className="border-border mt-4 border bg-black shadow-lg">
            ...
            <div className="px-4 pt-3 pb-4">
              <pre className="text-muted-foreground m-0 overflow-x-auto text-[11px] leading-relaxed break-words whitespace-pre-wrap">
                {error.stack}
...
function ContextBridge(props: Omit<ErrorBoundaryProps, "_ctx">) {
  ...
  return <ErrorBoundaryInner {...props} _ctx={ctxRef} />
}
