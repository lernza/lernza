...
const Skeleton = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(
  ({ className, ...props }, ref) => (
    <div
      ref={ref}
      className={cn("bg-muted border-border animate-pulse border", className)}
      {...props}
    />
  )
)
...
function SkeletonCard({ className }: { className?: string }) {
  return (
    <div className={cn("bg-card border-border border shadow-md", className)}>
  ...
  return (
    <div className={cn("grid grid-cols-2 gap-4 sm:grid-cols-4", className)}>
      {Array.from({ length: 4 }).map((_, i) => (
        <div key={i} className="bg-card border-border flex items-center gap-3 border p-4 shadow-md">
...
function SkeletonProfileHeader({ className }: { className?: string }) {
  return (
    <div className={cn("bg-card border-border overflow-hidden border shadow-lg", className)}>
      <div className="bg-accent relative h-20 sm:h-28">
        <div className="bg-diagonal-lines absolute inset-0 opacity-20" />
      </div>
      <div className="border-border border-t bg-white px-6 py-5">
        <div className="-mt-14 flex flex-col items-start gap-6 sm:-mt-16 sm:flex-row sm:items-center">
          <Skeleton className="border-border h-20 w-20 flex-shrink-0 border shadow-md" />
...
function SkeletonEarningsList({ count = 4, className }: { count?: number; className?: string }) {
  return (
    <div className={cn("space-y-4", className)}>
      {Array.from({ length: count }).map((_, i) => (
        <div key={i} className="bg-card border-border border p-5 shadow-md">
...
function SkeletonWelcomeBanner({ className }: { className?: string }) {
  return (
    <div className={cn("bg-accent border-border mb-8 border p-6 shadow-lg sm:p-8", className)}>
