...
describe("MOCK_QUESTS", () => {
  ...
  it("every quest has a unique id", () => {
    const ids = MOCK_QUESTS.map(w => w.id)
    expect(new Set(ids).size).toBe(ids.length)
  })

  it("every quest has a non-empty name and description", () => {
    for (const ws of MOCK_QUESTS) {
...
describe("MOCK_QUEST_STATS", () => {
  it("has a stats entry for every quest id", () => {
    for (const ws of MOCK_QUESTS) {
      expect(MOCK_QUEST_STATS[ws.id]).toBeDefined()
    }
  })
...
describe("MOCK_MILESTONES", () => {
  it("all milestone quest_ids reference an existing quest", () => {
    const questIds = new Set(MOCK_QUESTS.map(w => w.id))
...
describe("MOCK_COMPLETIONS", () => {
  it("all completion quest_ids reference an existing quest", () => {
    const questIds = new Set(MOCK_QUESTS.map(w => w.id))
...
describe("MOCK_ENROLLEES", () => {
  it("all enrollee quest_ids reference an existing quest", () => {
    const questIds = new Set(MOCK_QUESTS.map(w => w.id))
