...
if (isDev) {
  ...
  // Offline caching strategy is configured in vite.config.ts (#1626):
  // navigations + Stellar RPC use NetworkFirst (offline fallback),
  // static JS/CSS/images use StaleWhileRevalidate.
  import("virtual:pwa-register").then(({ registerSW }) => {
    registerSW({
      ...
      onOfflineReady() {
        ...
        window.dispatchEvent(new CustomEvent("lernza:offline-ready"))
      },
      onNeedRefresh() {
        if (isDev) console.info("[SW] new content available, will activate on next reload")
      },
    })
  })
}
